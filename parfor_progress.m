...
elseif N == 0
    ...
    progress = fscanf(f, '%d');
    fclose(f);
    iter = length(progress)-1;
    percent = iter/progress(1);
    
    if nargout == 0
        toPrint = "Iter: %s        Percentage %s %"; % E:\Optimisation - Thesis studies\Knee 5
        t0 = string(iter); t1 = string(percent); 
        toPrint= sprintf(toPrint,[t0,t1]);
