...
Hp.display("1. All Modules loaded!\n")
staFile = os.path.join("$workspacePath","PCKnee.sta")
if Hp.fileReader(staFile)[-1] == " THE ANALYSIS HAS COMPLETED SUCCESSFULLY\n":
     Hp.display("2. Passed if statement!\n")
     dataRet = os.path.join('/nobackup/mnsaz/AbqRunner',"dataRetrieval.py")
     command = 'abaqus python "%s"'%dataRet
     commandn = r'%s -- "%s"'%(command,"$workspacePath")
     pCall2 = subprocess.run(commandn, shell= True)
else:
     Hp.display("Job: %s did not solve"%("$inpPath"))
EOF
